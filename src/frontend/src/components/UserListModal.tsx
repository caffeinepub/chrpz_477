...
// Custom hooks for followers and following lists
function useGetFollowersList(userPrincipal: Principal) {
  const { actor, isFetching } = useActor();

  return useQuery<Principal[]>({
    queryKey: ["followersList", userPrincipal.toString()],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getFollowersList(userPrincipal);
    },
    enabled: !!actor && !isFetching,
  });
}

function useGetFollowingList(userPrincipal: Principal) {
  const { actor, isFetching } = useActor();

  return useQuery<Principal[]>({
    queryKey: ["followingList", userPrincipal.toString()],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getFollowingList(userPrincipal);
    },
    enabled: !!actor && !isFetching,
  });
}
