...
import type { Post, UserProfile, UserProfileInput, Comment, UserSelection } from "../backend";
...
// Get user profile with stats (for viewing other users)
export function useGetUserProfileWithStats(userPrincipal: Principal) {
  const { actor, isFetching } = useActor();

  return useQuery<UserProfile | null>({
    queryKey: ["userProfileWithStats", userPrincipal.toString()],
    queryFn: async () => {
      if (!actor) return null;
      return actor.getUserProfileWithStats(userPrincipal);
    },
    enabled: !!actor && !isFetching,
  });
}

// Chat User Selection List
export function useGetChatUserSelectionList() {
  const { actor, isFetching } = useActor();

  return useQuery<UserSelection[]>({
    queryKey: ["chatUserSelectionList"],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getChatUserSelectionList();
    },
    enabled: !!actor && !isFetching,
  });
}
