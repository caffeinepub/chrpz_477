...
actor {
  ...
  var conversations : Map.Map<Nat, Conversation> = Map.empty<Nat, Conversation>();
  var nextConversationId : Nat = 0;
  var nextMessageId : Nat = 0;

  public query ({ caller }) func getChatUserSelectionList() : async [UserSelection] {
    if (caller.isAnonymous()) {
      [];
    } else {
      let currentUserPrincipal = caller;
      let selectionList = List.empty<UserSelection>();

      userProfiles.keys().forEach(
        func(key) {
          if (key != currentUserPrincipal) {
            switch (userProfiles.get(key)) {
              case (?profile) {
                selectionList.add({
                  principal = key;
  ...
  public shared ({ caller }) func sendMessage(recipient : Principal, content : Text) : async () {
    ...
    if (not userProfiles.containsKey(recipient)) {
      Runtime.trap("Recipient does not have a user profile");
    };

    let participantsList = [caller, recipient];
    let existingConversationId = findExistingConversation(participantsList);
        ...
        switch (conversations.get(convId)) {
          case (?existingConversation) {
            ...
            let updatedConversation = {
              existingConversation with messages = updatedMessages
            };
            conversations.add(convId, updatedConversation);
          };
          case (null) {
  ...
  func findExistingConversation(participants : [Principal]) : ?Nat {
    let sortedParticipants = participants.sort(
      func(a, b) {
        if (a.toText() < b.toText()) { #less } else if (a.toText() > b.toText()) {
          #greater;
        } else { #equal };
      }
    );

    var foundConversationId : ?Nat = null;
    let it = conversations.entries();
    label convLoop loop {
      switch (it.next()) {
        case (?entry) {
          if (arrayEquals(entry.1.participants, sortedParticipants)) {
            foundConversationId := ?entry.0;
            break convLoop;
          };
        };
        case (null) {
  ...
  func arrayEquals(a : [Principal], b : [Principal]) : Bool {
  ...
  public query ({ caller }) func getConversation(conversationId : Nat) : async ?Conversation {
    ...
    switch (conversations.get(conversationId)) {
      case (?conversation) {
        let participantsIter = conversation.participants.values();
        if (not participantsIter.any(func(p) { p == caller })) {
  ...
  public query ({ caller }) func getConversationMessages(conversationId : Nat, sinceMessageId : ?Nat) : async [ChatMessage] {
    requireUserAuth(caller);
    switch (conversations.get(conversationId)) {
      case (?conversation) {
        let participantsIter = conversation.participants.values();
        ...
        switch (sinceMessageId) {
          case (?msgId) {
            conversation.messages.filter(func(msg) { msg.id > msgId });
          };
          case (null) {
